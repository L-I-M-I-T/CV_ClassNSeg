...
parser.add_argument('--dataset', default ='datasets', help='path to dataset')
# parser.add_argument('--dataset', default ='datasets/FaceForensics/selfreenactment', help='path to dataset')
parser.add_argument('--original', default ='original_sequences/youtube/c23/videos', help='original videos')
parser.add_argument('--mask', default ='manipulated_sequences/Face2Face/masks/videos', help='mask videos')
parser.add_argument('--altered', default ='manipulated_sequences/Face2Face/c23/videos', help='altered video')
parser.add_argument('--num_frames', type=int, default=20, help='Number of frames extracted for each video')
...
def extract_face_videos(input_path, output_path):
    ...
    f_img_original = os.path.join(output_path, 'original')
    f_img_altered = os.path.join(output_path, 'altered')
    ...
    for f in os.listdir(f_vid_mask):
        if os.path.isfile(os.path.join(f_vid_mask, f)):
            if f.lower().endswith(('mp4')):
                ...
                vidcap_original = cv2.VideoCapture(os.path.join(f_vid_original, f[:3]+".mp4"))
...
def extract_face_datasets(dataset, output):
    input_path = dataset
    output_path = os.path.join(output,"train")
