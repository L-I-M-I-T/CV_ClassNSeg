...
Author: Huy H. Nguyen
-----------------------------------------------------
Script for making FaceForensics++ FaceSwap dataset
"""
...
parser.add_argument('--input_fake', default ='datasets/FaceForensicsPP/test/c23/faceswap')
parser.add_argument('--mask', default ='datasets/FaceForensicsPP/test/masks/manipulated_sequences/FaceSwap/raw/masks')
parser.add_argument('--output_real', default ='datasets/faceswap/test/original')
parser.add_argument('--output_fake', default ='datasets/faceswap/test/altered')
    ...
    for f in os.listdir(input_fake):
        if os.path.isfile(os.path.join(input_fake, f)):
            if f.lower().endswith(('mp4')):
                ...
                vidcap_mask = cv2.VideoCapture(os.path.join(input_mask, filename))
                success_mask, image_mask = vidcap_mask.read()

                count = 0

                while (success_real and success_fake and success_mask):
                    ...
                    success_mask, image_mask = vidcap_fake.read()
